	...
	<div class="row">
		<div class="col-md-12">
			<div class="card border">
				...
				<?= form_open_multipart('admin/addDispatchingRawMaterial'); ?>
...
<?php if ($this->session->flashdata('SUCCESS_DISPATCHING_RAW')): ?>
	<script>
		Swal.fire({
			title: "Success",
			html: `<?= $this->session->flashdata('SUCCESS_DISPATCHING_RAW'); ?>`,
			icon: "success"
		});
	</script>
<?php endif; ?>
<?php if ($this->session->flashdata('FAILED_DISPATCHING_RAW')): ?>
	<script>
		document.addEventListener('DOMContentLoaded', function() {
			Swal.fire({
				title: "Error",
				html: `<?= $this->session->flashdata('FAILED_DISPATCHING_RAW'); ?>`,
				icon: "error"
			});
		});
	</script>
